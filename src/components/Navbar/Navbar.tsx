...
import { getCurrentPath, navigate } from '../../utils/router';
...
const SolutionsMegaDropdown: React.FC<{
  ...
  return (
    <div
      className="solutions-dropdown fixed mt-2.5 bg-white border border-[#E5E7EB] rounded-[20px] z-50 animate-in fade-in slide-in-from-top-2 duration-150 flex flex-col items-center isolation-isolate w-[94vw] max-w-[960px] left-0 right-0 mx-auto"
      style={{
        top: isScrolled ? '64px' : '68px',
        padding: '32px 0px 0px',
        gap: '28px',
        boxShadow: '0px 24px 48px -20px rgba(15, 23, 42, 0.18), 0px 4px 12px -4px rgba(15, 23, 42, 0.08)',
      }}
      onMouseLeave={() => { dropdownTimeout.current = setTimeout(onClose, 300); }}
      onMouseEnter={() => { if (dropdownTimeout.current) clearTimeout(dropdownTimeout.current); }}
    >
      {/* div.solutions-grid */}
      <div
        className="solutions-grid flex flex-row flex-wrap justify-center items-start p-0 w-full"
        style={{ padding: '0 32px', gap: '40px' }}
      >
        {/* Industry Section */}
        <div className="flex flex-col items-start p-0 w-full md:w-[423px]" style={{ gap: '14px' }}>
...
const FeaturesMegaDropdown: React.FC<{
  onClose: () => void;
  dropdownTimeout: React.MutableRefObject<ReturnType<typeof setTimeout> | null>;
  isScrolled: boolean;
}> = ({ onClose, dropdownTimeout, isScrolled }) => {
  ...
  return (
    <div
      className="features-dropdown fixed mt-2.5 bg-white border border-[#E5E7EB] rounded-[20px] z-50 animate-in fade-in slide-in-from-top-2 duration-150 flex flex-col items-center isolation-isolate w-[94vw] max-w-[920px] left-0 right-0 mx-auto"
      style={{
        top: isScrolled ? '64px' : '68px',
        padding: '32px 0px 0px',
        gap: '28px',
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col items-start p-0 flex-1 min-w-[260px] w-full md:w-auto" style={{ gap: '14px' }}>
          {/* div.mega-section-label */}
          <div className="mega-section-label flex flex-col items-start p-0 w-full" style={{ height: '16px' }}>
            <h4
              className="w-full flex items-center font-['Plus_Jakarta_Sans',sans-serif] font-bold text-[12.5px] leading-[16px] tracking-[0.625px] uppercase"
...
export const Navbar: React.FC<NavbarProps> = ({ currentPath: propPath }) => {
  const [isScrolled, setIsScrolled] = useState(false);
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const [localPath, setLocalPath] = useState(getCurrentPath());
  ...
  const cleanCurrent = (activePath || '').replace(/^[#/]+/, '').toLowerCase();
  const isDarkHeroPage = !isScrolled && (
    cleanCurrent === '' ||
    cleanCurrent === 'home' ||
    cleanCurrent === 'pricing' ||
    cleanCurrent === 'employee-app' ||
    cleanCurrent === 'hr-manager' ||
    cleanCurrent === 'help-center'
  );

  useEffect(() => {
    const handleScroll = () => setIsScrolled(window.scrollY > 20);
    const handlePopState = () => {
      setLocalPath(getCurrentPath());
    ...
    >
      ...
      >
        ...
        <div className="hidden lg:flex items-center justify-center gap-[4px] shrink-0">
          {NAV_LINKS.map((link) => {
            const isActive = activePath === link.href || link.dropdown?.some(sub => sub.href === activePath);
              ...
              return (
                ...
                >
                  <a
                    ...
                    className={`flex items-center gap-1.5 px-[12px] xl:px-[18px] py-[8px] font-['Plus_Jakarta_Sans',sans-serif] text-[13px] xl:text-[14.5px] leading-[20px] rounded-[14px] transition-all duration-200 cursor-pointer whitespace-nowrap ${isActive || isDropdownOpen
            ...
            return (
              <a
                ...
                href={link.href}
                onClick={() => window.scrollTo({ top: 0, left: 0, behavior: 'instant' })}
                className={`px-[12px] xl:px-[18px] py-[8px] font-['Plus_Jakarta_Sans',sans-serif] text-[13px] xl:text-[14.5px] leading-[20px] rounded-[14px] transition-all duration-200 whitespace-nowrap ${isActive
                  ? 'bg-white text-[#5B5FEF] font-bold shadow-[0_2px_8px_rgba(0,0,0,0.04)]'
                  : nonActiveStyle
                  }`}
              >
        ...
        <div className="flex-1 hidden lg:flex items-center justify-end gap-3 xl:gap-6 shrink-0 relative" ref={loginRef}>
          <button
            onClick={() => setIsLoginPopupOpen(!isLoginPopupOpen)}
            className={`font-['Plus_Jakarta_Sans',sans-serif] text-[14px] xl:text-[15px] transition-colors cursor-pointer ${isScrolled
              ? 'text-[#555C6E] font-semibold'
              : isDarkHeroPage
          ...
          <a
            href="/contact-support"
            className="bg-[#5B5FEF] hover:bg-[#4F46E5] text-white font-['Plus_Jakarta_Sans',sans-serif] text-[14px] xl:text-[15px] font-bold leading-[18px] px-[18px] xl:px-[26px] py-[10px] xl:py-[11px] rounded-full transition-all duration-200 shadow-sm shadow-[#5B5FEF]/30 cursor-pointer whitespace-nowrap">
            Book Demo
          </a>
          <ComingSoonPopup isOpen={isLoginPopupOpen} onClose={() => setIsLoginPopupOpen(false)} />
        </div>
